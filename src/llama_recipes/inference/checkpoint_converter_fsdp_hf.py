...
from llama_recipes.inference.model_utils import  load_llama_from_config, load_automodel_from_config
...
def main(
    ...
    is_lama_model = 'llama' in HF_model_path_or_name
    #load the HF model definition from config
    if is_lama_model:
        model_def = load_llama_from_config(HF_model_path_or_name)
    else:
        model_def = load_automodel_from_config(HF_model_path_or_name)
